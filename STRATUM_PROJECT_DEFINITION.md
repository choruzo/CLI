...
- [x] Tool `question` para la tanda única de preguntas al usuario que contempla el prompt de opencode (F7 del reporte) — cerrado 2026-08-27: tool de control (`src/tools/question.ts`) interceptada por el `ReactLoop` como `present_plan`, nunca despachada. Máximo 4 preguntas, una sola tanda por run (la segunda llamada → `tool_error` recuperable). El gate se resuelve con `RunOptions.onAskQuestions`: `<QuestionPrompt>` en el chat Ink (selector de opciones + texto libre, Esc omite), readline en `stratum run`/`stratum init` (`src/cli/ask-questions.ts`) y **sin TTY no hay callback** — el loop inyecta `<question_answers unavailable="true">` instruyendo a continuar con supuestos, nunca bloquea. Eventos `questions_asked`/`questions_answered`; visible también en Fase 1 de plan (`PLAN_ALLOWLIST`) y oculta a los subagentes (la TTY es del padre). Bloque "Asking the user" añadido al `INITIALIZE_PROMPT`. Verificado de punta a punta con el modelo local (`qwen3.6-35b-a3b` vía LiteLLM): (a) `stratum run` sin TTY — el modelo llama `question`, el loop la intercepta y el agente continúa con supuestos; (b) gate resuelto con respuestas — el modelo emite la tanda con opciones y usa las respuestas inyectadas en su salida final
