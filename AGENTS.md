...
Cliente SSH nativo sobre `ssh2` — el binario `ssh` del sistema nunca se invoca (§12.14). Las tools `ssh_exec`, `ssh_upload` y `ssh_download` **solo se registran si `.stratumrc.json` trae sección `ssh` con al menos un host**: sin inventario, el LLM no las ve.

- `pool.ts` — `SSHConnectionPool`: conexiones persistentes por alias, apertura lazy. El mapa `inflight` es el mutex de establecimiento (dos `ssh_exec` paralelos al mismo alias comparten socket, necesario porque va con `serialized: false`). Jump hosts vía `forwardOut` (profundidad máx. 2, validada en el schema). Reconexión con backoff 2s→4s→8s **solo** para conexiones ya establecidas que se caen; el primer fallo de apertura rechaza de inmediato. `closeAll()` cierra hojas antes que bastiones. Listener de `error` permanente: ssh2 emite errores tardíos que sin manejador tumban el proceso
...
- `exec.ts` — `ssh_exec`. Confirmación vía `isDestructive()`: `confirmAll` del host, o patrón destructivo (reutiliza `commandIsDestructive` de `shell/bash.ts`). Corta la salida en `maxBytes` (256 KB) y el comando en `commandTimeout` (30 s) matando el proceso remoto con `stream.signal('KILL')`. Devuelve XML `<ssh_result>`
...
- `audit.ts` — `~/.stratum/logs/ssh-audit.jsonl`, un registro por comando, rotación a 10 MB. El `sessionId` viaja por `RunOptions`→`ToolContext`: `chat` lo genera **al arrancar** (no al guardar, como antes), `run` usa uno efímero por invocación, y los subagentes heredan el del padre
