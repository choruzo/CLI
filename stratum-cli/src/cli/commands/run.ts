...
export const runCommand = new Command('run')
  ...
  .action(async (task: string, opts: { provider?: string; allowDestructive?: boolean; denyDestructive?: boolean }) => {
    let config;
    try {
      config = loadConfig();
    } catch (err) {
    ...
    try {
      for await (const event of agent.run(task, {
        ...
        switch (event.type) {
          ...
          case 'tool_result': {
            const duration = ((Date.now() - (toolStartTimes.get(event.id) ?? Date.now())) / 1000).toFixed(1);
            const label = summarizeInput(
              (() => { try { return JSON.parse(event.result.slice(0, 200)); } catch { return {}; } })()
